...
<div class="w-full h-full flex flex-col md:flex-row items-center justify-between p-2 sm:p-4 md:p-6 gap-3 md:gap-6 select-none font-sans overflow-hidden">
    ...
    <div class="flex-1 w-full h-full flex flex-col items-center justify-center relative bg-white/40 border border-black/5 overflow-hidden font-mono">
        {#if gamePhase === "idle"}
            <button
                onclick={start}
                class="w-full h-full bg-white text-text p-4 sm:p-6 flex flex-col items-center justify-center gap-2 sm:gap-3 hover:bg-primary hover:text-white transition-colors cursor-pointer group shadow-xs"
            >
                <span class="material-symbols-outlined text-[40px] sm:text-[48px] text-primary group-hover:text-white transition-colors">
                    bolt
                </span>
                ...
                <span class="mt-1 sm:mt-2 px-4 py-1.5 bg-text/5 group-hover:bg-white group-hover:text-primary text-text font-bold text-[11px] sm:text-xs uppercase tracking-widest transition-colors">
                    INITIALIZE TEST
                </span>
            </button>
        {:else if gamePhase === "waiting"}
            <button
                onclick={handleClick}
                class="w-full h-full bg-amber-50 text-amber-950 p-4 sm:p-6 flex flex-col items-center justify-center gap-2 sm:gap-3 cursor-pointer"
            >
                <span class="material-symbols-outlined text-[36px] sm:text-[44px] text-amber-600 animate-spin">
                    progress_activity
                </span>
        ...
        {:else if gamePhase === "ready"}
            <button
                onclick={handleClick}
                class="w-full h-full bg-primary text-white p-4 sm:p-6 flex flex-col items-center justify-center gap-2 cursor-pointer animate-pulse shadow-lg"
            >
                <span class="material-symbols-outlined text-[48px] sm:text-[60px]">
                    touch_app
                </span>
                <h4 class="text-3xl sm:text-4xl md:text-5xl font-black tracking-widest uppercase">TRIGGER NOW!</h4>
            </button>
        {:else if gamePhase === "clicked"}
            <div class="w-full h-full bg-white text-text p-4 sm:p-6 flex flex-col items-center justify-center gap-2 text-center shadow-xs">
                ...
                <button
                    onclick={start}
                    class="mt-2 sm:mt-3 px-4 py-1.5 sm:px-5 sm:py-2 bg-primary hover:bg-primary/90 text-white text-[11px] sm:text-xs font-bold uppercase tracking-wider transition cursor-pointer flex items-center gap-1.5 shadow-md shadow-primary/20"
                >
        ...
        {:else if gamePhase === "tooEarly"}
            <div class="w-full h-full bg-rose-50 text-rose-950 p-4 sm:p-6 flex flex-col items-center justify-center gap-2 text-center">
                <span class="material-symbols-outlined text-[28px] sm:text-[32px] text-rose-600">
                    warning
                </span>
                ...
                <button
                    onclick={start}
                    class="mt-2 sm:mt-3 px-4 py-1.5 sm:px-5 sm:py-2 bg-rose-600 hover:bg-rose-700 text-white text-[11px] sm:text-xs font-bold uppercase tracking-wider transition cursor-pointer shadow-md"
                >
                    RESTART SENSOR
                </button>
            </div>
