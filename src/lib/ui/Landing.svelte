...
<div class="columns-2 relative w-full h-full min-h-dvh md:h-screen gap-8 max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8">
    

    <div class="flex flex-col justify-between items-center h-full select-none overflow-hidden">
        <!-- Top Spacer for Floating Navbar -->
        <div class="h-8 sm:h-12"></div>

        <!-- Center Hero Title & Subtitle & Brand CTA -->
        <div class="relative z-10 flex flex-col items-start justify-center my-auto text-left max-w-5xl px-4">


            <!-- Main Headline inspired by reference design -->
            <h1
                class="text-xl sm:text-6xl md:text-7xl lg:text-7xl font-black tracking-tight leading-[1.02] transition-all duration-1000 ease-out {isLoaded ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}"
