...
import { mat, env, floorGrid, surface } from './materials'
import { seg, count, QUALITY } from './quality'
import {
  makeCarpet,
  makeCounter,
  makePlant,
...
export function makeHall({ accent = '#2f6bff' } = {}) {
  ...
  deskPost.position.set(0, 1.5, -0.06)
  ...
  for (const x of [-11, 11]) {
    for (const z of [-HALL.aisleHalf - 0.6, HALL.aisleHalf + 0.6]) {
      ...
      hall.add(planter)
      solid(x, z, planter.userData.halfW || 1.1, planter.userData.halfD || 0.32)
    }
  }
...
export function makeGenericBooth({ name, color, stand, style = 'wall' }) {
  ...
  const DECK = 0.02
  const carpet = makeCarpet(W, D, null, { riser: DECK, accent: tint, color: tint })
  g.add(carpet)

  if (style !== 'island') {
    const wall = box(W, H, 0.14, mat.panel(0xf4f5f8, 0.7, 4))
    wall.position.set(0, DECK + H / 2, -D / 2 + 0.1)
    g.add(wall)
    ...
    const band = box(
      ...
      H * 0.34,
      0.06,
      new THREE.MeshStandardMaterial({ color: tint, roughness: 0.55, metalness: 0.05, envMapIntensity: env(0.5) })
    )
    band.position.set(0, DECK + H * 0.62, -D / 2 + 0.2)
    g.add(band)
    ...
    sign.position.set(0, DECK + H * 0.32, -D / 2 + 0.21)
    g.add(sign)
  }

  if (style === 'tower') {
    const tower = roundedBox(1.1, 4.2, 1.1, 0.05, mat.panel(0xfbfbfa, 0.5, 3))
    tower.position.set(-W / 2 + 1.2, DECK + 2.1, -D / 2 + 1.4)
    g.add(tower)
    const towerBand = box(
      1.16,
      1.2,
      1.16,
      new THREE.MeshStandardMaterial({ color: tint, roughness: 0.5, envMapIntensity: env(0.5) })
    )
    towerBand.position.set(-W / 2 + 1.2, DECK + 3.1, -D / 2 + 1.4)
    g.add(towerBand)
  }
  ...
  if (style === 'island') {
    ...
    const ring = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: tint, roughness: 0.4, metalness: 0.2, envMapIntensity: env(0.5) })
    )
    ...
    const POD = { rTop: 1.25, rBot: 1.4, h: 1.05, y: DECK + 0.52, z: -0.6 }
  ...
  counter.position.set(W / 2 - 1.4, DECK, D / 2 - 1.6)
  counter.rotation.y = -0.3
  g.add(counter)

  const plant = makePlant({ height: 1.0, leaves: 12 })
  plant.position.set(-W / 2 + 0.7, DECK, D / 2 - 0.9)
  g.add(plant)
  ...
  for (const x of [-TRUSS.legX, TRUSS.legX]) {
    ...
    leg.position.set(x, (DECK + TRUSS.y - 0.19) / 2 + 0.11, TRUSS.z)
    g.add(leg)
  }
  ...
  const platePost = new THREE.Mesh(
    new THREE.CylinderGeometry(0.028, 0.034, PLATE.y - DECK, seg(12, 6)),
    mat.metal(0xb2b7c0, 0.32, 1)
  )
  platePost.position.set(PLATE.x + 0.06, DECK + (PLATE.y - DECK) / 2, PLATE.z)
  g.add(platePost)
...
function makeFlightCases(accent) {
  ...
  for (const s of sizes) {
    ...
    for (const sx of [-1, 1]) {
      ...
      const at = new THREE.Vector3((sx * s.w) / 2, 0, s.d / 2).applyAxisAngle(UP, s.rot)
      rail.position.set(s.x + at.x, s.y, at.z)
    ...
    const stripeAt = new THREE.Vector3(0, 0, s.d / 2 + 0.002).applyAxisAngle(UP, s.rot)
    stripe.position.set(s.x + stripeAt.x, s.y + s.h * 0.22, stripeAt.z)
